...
def get_user_data(username):
    ...
    conn = sqlite3.connect(DB_PATH)
    conn.execute(
        "INSERT OR IGNORE INTO user_stats (username, moedas) VALUES (?, 0)", (username,)
    )
    conn.execute("INSERT OR IGNORE INTO user_config (username) VALUES (?)", (username,))
    conn.commit()

    stats = conn.execute(
        "SELECT moedas FROM user_stats WHERE username = ?", (username,)
    ).fetchone()
    config = conn.execute(
        "SELECT selected_skin, selected_bg FROM user_config WHERE username = ?",
        (username,),
    ).fetchone()
    inventory = conn.execute(
        "SELECT item_id FROM inventory WHERE username = ?", (username,)
    ).fetchall()
    conn.close()
...
def buy_item(username, item_id, item_type, price):
    ...
    if moedas >= price:
        conn.execute(
            "UPDATE user_stats SET moedas = moedas - ? WHERE username = ?",
            (price, username),
        )
        conn.execute(
            "INSERT OR IGNORE INTO inventory (username, item_id, item_type) VALUES (?, ?, ?)",
...
def select_item(username, item_id, item_type):
    ...
    if item_type == "skin":
        conn.execute(
            "UPDATE user_config SET selected_skin = ? WHERE username = ?",
            (item_id, username),
        )
    else:
        conn.execute(
            "UPDATE user_config SET selected_bg = ? WHERE username = ?",
            (item_id, username),
        )
    conn.commit()
    conn.close()
