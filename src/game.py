...
from src.constants import (
    LARGURA, ALTURA,
    verde, verde_claro, vermelho, vermelho_claro, roxo_claro,
...
class Game:
    ...
    def game_intro(self):
        # ← CORREÇÃO: aguarda o mouse ser solto antes de processar qualquer clique.
        ...
        while True:
            ...
            if button(self.display, "SCORES", 565, 720, 370, 160, vermelho,    vermelho_claro,      'scores') == 'scores':
                self.show_scores()

            if button(self.display, "LOJA",  1000, 720, 370, 160, ouro,    (255, 255, 100), 'shop')   == 'shop':
                username = self.get_username()
    ...
    def show_shop(self, username):
        ...
        while True:
            ...
            for i, item in enumerate(items):
                ...
                is_equipped = (user_data["skin"] == item["id"] if current_tab == "skin"
            ...
            if button(self.display, "VOLTAR PARA O MENU", LARGURA//2 - 300, 850, 600, 100, vermelho, vermelho_claro, 'back') == 'back':
                return
    ...
    def show_scores(self):
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key in (pygame.K_ESCAPE, pygame.K_BACKSPACE):
                        return
                    elif event.key == pygame.K_LEFT  and has_prev: page -= 1
                    elif event.key == pygame.K_RIGHT and has_next: page += 1
            ...
            i_surf = info_font.render(
                f"Página {page + 1} de {total_pages}   •   {total} partidas", True, prata
            )
            self.display.blit(i_surf, i_surf.get_rect(center=(LARGURA // 2, 148)))

            col_font = pygame.font.Font('freesansbold.ttf', 40)
            for label, x in [("#", 80), ("JOGADOR", 240), ("PONTOS", 820), ("DATA", 1120)]:
            ...
            for i, (username, timestamp, pontos) in enumerate(rows):
                ...
                cor = ouro if global_rank == 0 else prata if global_rank == 1 else bronze if global_rank == 2 else branco
                y   = 258 + i * 72
            ...
            if button(self.display, "< ANTERIOR", 80,   865, 350, 90, ic_p, azul_claro if has_prev else cinza, 'prev') == 'prev' and has_prev:
                page -= 1
            ...
            if button(self.display, "PROXIMA >", 1070, 865, 350, 90, ic_n, azul_claro if has_next else cinza, 'next') == 'next' and has_next:
                page += 1
    ...
    def run(self, username):
        ...
        user_data  = get_user_data(username)
        skin_img   = self.assets.get(user_data.get("skin", "bird_img"), self.assets['bird_img'])
